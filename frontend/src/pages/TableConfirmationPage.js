...
const TableConfirmationPage = () => {
      ...
      try {
        ...
        if (reservationId) {
          ...
          if (response.data.success && response.data.reservation) {
            ...
            throw new Error(
              response.data.error || "Failed to fetch reservation details"
            );
          }
        } else {
  ...
  const generateInvoiceNumber = () => {
    return `INV-${reservation._id.substring(0, 8)}-${Math.floor(
      Math.random() * 1000
    )}`;
  };
  ...
  if (showInvoice) {
    ...
    return (
      <PageLayout>
        <div
          className={`invoice-page ${isMobile() ? "mobile-invoice" : ""}`}
          style={{ paddingTop: isMobile() ? "80px" : "2rem" }}
        >
          ...
          <div className="invoice-container" ref={invoiceRef}>
            <div className="invoice-header">
              ...
              <div className="invoice-details">
                <h2>INVOICE</h2>
                <p>
                  <strong>Invoice No:</strong> {generateInvoiceNumber()}
                </p>
                <p>
            ...
            <div className="invoice-to">
              ...
              <p>
                <strong>
                  {userData?.name || reservation.fullName || "Guest"}
                </strong>
              </p>
                ...
                <tbody>
                  <tr>
                    <td data-label="Description">
                      ...
                      <br />
                      <small>
                        Time: {reservation.time} - {reservation.endTime}
                      </small>
                    </td>
                    ...
                    <td data-label="Amount">
                      Rs.{" "}
                      {parseInt(reservation.totalPrice).toLocaleString("en-PK")}
                    </td>
                  </tr>
      ...
      <div className="confirmation-hero">
        <div className="hero-content">
          ...
          <p className="hero-subtitle">
            Your dining experience is confirmed and ready
          </p>
          <div className="reservation-id-badge">
            <FiHash className="badge-icon" />
            <span>
              Reservation ID:{" "}
              {reservation._id?.substring(0, 8).toUpperCase() || "TEMP"}
            </span>
          </div>
        </div>
      </div>
                ...
                <div className="detail-body">
                  <div className="detail-row">
                    ...
                    <span className="value">
                      {userData?.name || reservation.fullName || "N/A"}
                    </span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Email:</span>
                    <span className="value">
                      {userData?.email || reservation.email || "N/A"}
                    </span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Phone:</span>
                    <span className="value">
                      {reservation.phone || userData?.phone || "N/A"}
                    </span>
                  </div>
                </div>
              </div>
