...
class TableMLModelLoader {
  ...
  async loadDatasets() {
    try {
      ...
      try {
        ...
        console.log(
          "⚠️ Database loading failed, using mock data:",
          dbError.message
        );
        this.datasets = {
  ...
  async loadRealTableData() {
    try {
      ...
      console.log(
        `✅ Loaded ${processedTables.length} real tables for ML model`
      );
      return processedTables.length > 0
        ? processedTables
        : this.createMockTablesData();
    } catch (error) {
      ...
      for (const user of realUsers) {
        ...
        const avgPartySize =
          interactions.length > 0
            ? interactions.reduce(
                (sum, i) => sum + (i.context?.partySize || 2),
                0
              ) / interactions.length
            : 2;
        ...
        const mostCommonOccasion =
          Object.keys(preferredOccasions).length > 0
            ? Object.keys(preferredOccasions).reduce((a, b) =>
                preferredOccasions[a] > preferredOccasions[b] ? a : b
              )
            : "Casual";
        ...
        processedUsers.push({
          ...
          prefersPrivate: interactions.some(
            (i) => i.context?.occasion === "Romantic"
          ),
        });
      }

      console.log(`✅ Loaded ${processedUsers.length} real users for ML model`);
      return processedUsers.length > 0
        ? processedUsers
        : this.createMockUsersData();
    } catch (error) {
  ...
  async loadRealInteractionsData() {
    try {
      ...
      console.log(
        `✅ Loaded ${processedInteractions.length} real interactions for ML model`
      );
      return processedInteractions.length > 0
        ? processedInteractions
        : this.createMockInteractionsData();
    } catch (error) {
