...
class MLModelLoader {
  ...
  async loadModels() {
    try {
      ...
      const integrationPath = path.join(
        this.modelPath,
        "hrms_integration_package.json"
      );
      if (fs.existsSync(integrationPath)) {
        this.integrationConfig = JSON.parse(
      ...
      const modelPath = path.join(
        this.modelPath,
        "complete_food_recommendation_model.pkl"
      );
      ...
      if (fs.existsSync(mappingsPath) && fs.existsSync(modelPath)) {
        ...
        console.log(
          "⚠️ Real SVD model files not found, using mock implementation"
        );
  ...
  getModelInfo() {
    ...
    return {
      ...
      pakistaniCuisine:
        this.integrationConfig?.pakistani_cuisine_adaptations || {},
  ...
  getPakistaniRecommendations(count = 10) {
    ...
    const popularDishes =
      this.integrationConfig.pakistani_cuisine_adaptations?.popular_dishes ||
      [];
  ...
  saveUserInteraction(userId, menuItemId, rating, interactionType = "rating") {
    try {
      ...
      console.log(
        `💾 Saved interaction for user ${userId}: ${menuItemId} rated ${rating}`
      );
      return true;
    } catch (error) {
  ...
  updateUserProfile(userId, preferences) {
    try {
      ...
      fs.writeFileSync(
        profilesPath,
        JSON.stringify(this.userProfiles, null, 2)
      );
  ...
  async initializeRealSVDModel() {
    try {
      ...
      const healthResponse = await axios.get(
        `${this.pythonModelService}/health`
      );
      if (healthResponse.data.status === "healthy") {
        ...
        const loadResponse = await axios.post(
          `${this.pythonModelService}/load_model`
        );
        if (loadResponse.data.success) {
          ...
          const accuracyResponse = await axios.get(
            `${this.pythonModelService}/accuracy`
          );
          if (accuracyResponse.data.success) {
            console.log(
              "📈 Real Model Accuracy:",
              accuracyResponse.data.accuracy_metrics
            );
          }
      ...
      console.log(
        "⚠️ Failed to connect to Python model service:",
        error.message
      );
  ...
  async predictRating(userId, itemId) {
    ...
    if (this.realModelLoaded) {
      try {
        const response = await axios.post(
          `${this.pythonModelService}/predict`,
          {
            user_id: userId,
            recipe_id: itemId,
          }
    ...
    if (!userFactors || !itemFactors) {
      ...
      const userMean =
        this.svdModel.userMeans.get(userId) || this.svdModel.globalMean;
      const itemMean =
        this.svdModel.itemMeans.get(itemId) || this.svdModel.globalMean;
  ...
  async generateSVDRecommendations(userId, candidateItems, count = 10) {
    ...
    if (this.realModelLoaded) {
      try {
        ...
        const response = await axios.post(
          `${this.pythonModelService}/recommendations`,
          {
            user_id: userId,
          ...
          return recommendations
            .map((rec) => {
              const item = candidateItems.find(
                (item) => (item._id || item.id) === rec.recipe_id
              );
        ...
        console.log(
          "Real model recommendations failed, using mock:",
          error.message
        );
      }
    }
    ...
    return predictions
      ...
      .map((pred) => ({
        ...
        reason: this.realModelLoaded
          ? "real_svd_collaborative_filtering"
          : "mock_svd_collaborative_filtering",
