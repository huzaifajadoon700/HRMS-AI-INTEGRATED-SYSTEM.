 ...
 * Handles real-time order updates, notifications, and live system events
 *
 * @description Manages WebSocket connections for the HRMS system
 * @version 1.0.0
 */
...
const updateOrderStatus = async (orderId, status) => {
  ...
  console.log(
    `[${currentTime.toLocaleTimeString()}] Order ${orderId} status updated to: ${status}`
  );
  ...
  try {
    ...
    if (updatedOrder) {
      console.log(
        `[DB] Order ${orderId} updated to ${status} status in database`
      );
    }
  } catch (error) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("trackOrder", async ({ orderId }) => {
      ...
      if (activeOrderProgression.has(orderId)) {
        console.log(
          `[Socket] Order ${orderId} already being tracked, joining room`
        );
      ...
      socket.on("disconnect", () => {
        ...
        if (!room || room.size === 0) {
          console.log(
            `[Socket] No more clients tracking order ${orderId}, cleaning up`
          );
          activeOrderProgression.delete(orderId);
          timeouts.forEach((t) => clearTimeout(t));
        }
