...
async function fixEvaluationData() {
  ...
  try {
    ...
    console.log(
      `📊 Found ${existingInteractions.length} existing interactions`
    );
    ...
    for (const interaction of existingInteractions) {
      if (!interaction.rating) {
        ...
        await UserFoodInteraction.findByIdAndUpdate(interaction._id, {
          rating,
        });
        updatedCount++;
      }
    ...
    if (users.length > 0 && menuItems.length > 0) {
      ...
      for (let day = 0; day < 3; day++) {
        ...
        for (const user of users) {
          ...
          for (let i = 0; i < interactionCount; i++) {
            ...
            const interactionType = ["view", "order", "rating"][
      ...
      console.log(
        `✅ Created ${recentInteractions.length} recent interactions for evaluation`
      );
    }

    // Verify evaluation data
    const totalInteractions = await UserFoodInteraction.countDocuments({});
    const ratedInteractions = await UserFoodInteraction.countDocuments({
      rating: { $exists: true },
    });
    ...
    console.log(
      `Recent Rated Interactions (7 days): ${recentRatedInteractions}`
    );
