  ...
  try {
    ...
    for (const user of users) {
      ...
      for (let i = 0; i < userInteractionCount; i++) {
        ...
        const interaction = {
          ...
          rating: ["rating", "order"].includes(interactionType)
            ? rating
            : undefined,
    ...
    const summary = {
      ...
      interactionTypes: {
        ...
        rating: interactions.filter((i) => i.interactionType === "rating")
          .length,
        favorite: interactions.filter((i) => i.interactionType === "favorite")
          .length,
      },
      averageInteractionsPerUser: Math.round(
        interactions.length / users.length
      ),
      dateRange: {
    ...
    console.log(
      `Average Interactions per User: ${summary.averageInteractionsPerUser}`
    );
    console.log(
