// Food Recommendation Controller - Manages ML-based food recommendations and user interactions
...
class FoodRecommendationController {
  ...
  static async getRecommendations(req, res) {
    try {
      ...
      res.json({
        ...
        cached: false,
        userStats: {
          totalInteractions: await UserFoodInteraction.countDocuments({
            userId,
          }),
  ...
  static async generateRecommendations(userId, count = 10) {
    try {
      ...
      if (userInteractions.length === 0) {
        console.log(
          "🆕 New user detected, generating popularity-based recommendations"
        );
      ...
      const svdRecs = await FoodRecommendationController.getSVDRecommendations(
        userId,
        allMenuItems,
        Math.ceil(count * 0.5)
      );
      ...
      const uniqueRecommendations =
        FoodRecommendationController.advancedDeduplication(allRecommendations);
  ...
  static async getPopularityBasedRecommendations(count) {
    ...
    if (popularItems.length === 0) {
      console.log(
        "⚠️ No items with availability:true, trying all available items..."
      );
      popularItems = await Menu.find({ availability: { $ne: false } })
        .sort({ averageRating: -1, totalRatings: -1 })
        .limit(count);
      console.log(
        "📊 Found items without strict availability filter:",
        popularItems.length
      );
    }
  ...
  static async getSVDRecommendations(userId, candidateItems, count) {
    try {
      ...
      const svdRecommendations = await mlModelLoader.generateSVDRecommendations(
        userId,
        candidateItems,
        count
      );
  ...
  static async getCollaborativeRecommendations(userId, userPreferences, count) {
    try {
      ...
      userSimilarity.forEach((userData, otherUserId) => {
        const avgRating =
          userData.interactions.reduce((sum, int) => sum + int.rating, 0) /
      ...
      const sortedSimilarUsers = Array.from(userSimilarity.entries())
        .sort(
          (a, b) =>
            Math.abs(a[1].avgRating - userAvgRating) -
            Math.abs(b[1].avgRating - userAvgRating)
        )
        ...
        userData.interactions.forEach((interaction) => {
          if (
            ...
            const similarityScore =
              1 - Math.abs(userData.avgRating - userAvgRating) / 4; // Normalize to 0-1
      ...
      return recommendedItems.sort((a, b) => b.score - a.score).slice(0, count);
    } catch (error) {
  ...
  static async getContentBasedRecommendations(userPreferences, count) {
    try {
      ...
      const preferredSpiceLevels = Object.entries(
        userPreferences.preferredSpiceLevels || {}
      )
        .sort((a, b) => b[1] - a[1])
        .map(([spiceLevel]) => spiceLevel);

      // Get user's preferred dietary tags
      const preferredDietaryTags = Object.entries(
        userPreferences.preferredDietaryTags || {}
      )
        .sort((a, b) => b[1] - a[1])
        .map(([tag]) => tag);
      ...
      allResults.forEach((result) => {
        ...
        if (
          !uniqueItems.has(itemId) ||
          uniqueItems.get(itemId).matchScore < result.matchScore
        ) {
  ...
  static calculateContentMatchScore(item, userPreferences) {
    ...
    if (item.cuisine && cuisinePrefs[item.cuisine]) {
      ...
      const totalCuisineInteractions = Object.values(cuisinePrefs).reduce(
        (sum, freq) => sum + freq,
        0
      );
    ...
    if (item.category && categoryPrefs[item.category]) {
      ...
      const totalCategoryInteractions = Object.values(categoryPrefs).reduce(
        (sum, freq) => sum + freq,
        0
      );
    ...
    if (item.spiceLevel && spicePrefs[item.spiceLevel]) {
      ...
      const totalSpiceInteractions = Object.values(spicePrefs).reduce(
        (sum, freq) => sum + freq,
        0
      );
    ...
    if (item.dietaryTags && item.dietaryTags.length > 0) {
      ...
      const totalDietaryInteractions = Object.values(dietaryPrefs).reduce(
        (sum, freq) => sum + freq,
        0
      );
    ...
    if (item.averageRating && item.totalRatings) {
      const qualityScore =
        (item.averageRating / 5) * Math.min(1, item.totalRatings / 10);
  ...
  static advancedDeduplication(recommendations) {
    ...
    recommendations.forEach((rec) => {
      ...
      if (itemMap.has(itemId)) {
        ...
        const combinedScore =
          existing.score * existing.weight + rec.score * rec.weight;
  ...
  static async getPakistaniRecommendations(req, res) {
    try {
      ...
      const pakistaniItems = await Menu.find({
        ...
        $or: [
          ...
          { category: { $regex: /pakistani/i } },
          {
            name: {
              $regex:
                /(biryani|karahi|kebab|nihari|haleem|pulao|tikka|naan|dal)/i,
            },
          },
      ...
      if (userId && userId !== "guest") {
        ...
        if (userInteractions.length > 0) {
          userPreferences =
            FoodRecommendationController.analyzeUserPreferences(
              userInteractions
            );
        }
      }
      ...
      const scoredItems = pakistaniItems.map((item) => {
        ...
        if (
          userPreferences.preferredSpiceLevels &&
          userPreferences.preferredSpiceLevels[item.spiceLevel]
        ) {
          score += 0.5;
          confidence = "high";
        ...
        if (
          userPreferences.preferredCategories &&
          userPreferences.preferredCategories[item.category]
        ) {
          score += 0.3;
          confidence = "high";
        }
