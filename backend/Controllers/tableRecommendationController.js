// Table Recommendation Controller - Handles ML-based table recommendations and user interactions
...
const getTableRecommendations = async (req, res) => {
  try {
    ...
    if (
      isNaN(validatedPartySize) ||
      validatedPartySize < 1 ||
      validatedPartySize > 20
    ) {
      return res.status(400).json({
        success: false,
    ...
    const allAvailableTables = await Table.find({ status: "Available" }).sort({
      avgRating: -1,
      totalBookings: -1,
    });
    ...
    try {
      if (tableMLLoader.isLoaded()) {
        ...
        usingMLModel = true;
        console.log(
          `🤖 ML Model provided ${mlRecommendations.length} recommendations`
        );
      }
    } catch (mlError) {
    ...
    if (mlRecommendations.length < parseInt(numRecommendations)) {
      console.log(
        `🔄 Supplementing with smart matching (need ${
          parseInt(numRecommendations) - mlRecommendations.length
        } more)`
      );
      ...
      const smartMatchedTables = await Table.find(matchCriteria).sort({
        avgRating: -1,
        totalBookings: -1,
      });
      const tablesToUse =
        smartMatchedTables.length > 0 ? smartMatchedTables : allAvailableTables;

      // Get existing table IDs from ML recommendations
      const existingTableIds = new Set(
        mlRecommendations.map((rec) => rec.tableId)
      );
      ...
      const additionalRecommendations = additionalTables.map(
        (table, index) => ({
          tableId: table._id.toString(),
          score: usingMLModel
            ? 0.6 + Math.random() * 0.2
            : (additionalTables.length - index) / additionalTables.length,
    ...
    const enrichedRecommendations = await Promise.all(
      mlRecommendations
        .slice(0, parseInt(numRecommendations))
        .map(async (rec, index) => {
          let table = null;
          ...
          if (
            table.capacity >= validatedPartySize &&
            table.capacity <= validatedPartySize + 2
          ) {
            explanationFactors.push(
              `perfect size for ${validatedPartySize} guests`
            );
          }

          if (
            validatedOccasion === "Romantic" &&
            ["Intimate", "Romantic", "Quiet"].includes(table.ambiance)
          ) {
            explanationFactors.push(
              `${table.ambiance.toLowerCase()} ambiance ideal for romantic dining`
            );
          } else if (
            validatedOccasion === "Business" &&
            ["Formal", "Quiet"].includes(table.ambiance)
          ) {
            explanationFactors.push(
              `${table.ambiance.toLowerCase()} setting perfect for business meetings`
            );
          } else if (
            validatedOccasion === "Celebration" &&
            ["Lively", "Social"].includes(table.ambiance)
          ) {
            explanationFactors.push(
              `${table.ambiance.toLowerCase()} atmosphere great for celebrations`
            );
          }
          ...
          if (table.avgRating >= 4.0) {
            explanationFactors.push(
              `highly rated (${table.avgRating}/5 stars)`
            );
          }
          ...
          return {
            ...
            confidence:
              rec.confidence ||
              (explanationFactors.length >= 2 ? "high" : "medium"),
            rank: index + 1,
...
const getPopularTables = async (req, res) => {
  try {
    ...
    if (popularTables.length < requestedLimit) {
      console.log(
        `⚠️ Only ${popularTables.length} available tables, supplementing with all tables`
      );
      ...
      const existingIds = new Set(
        popularTables.map((table) => table._id.toString())
      );

      // Add non-duplicate tables
      const additionalTables = allTables.filter(
        (table) => !existingIds.has(table._id.toString())
      );

      popularTables = [...popularTables, ...additionalTables].slice(
        0,
        requestedLimit
      );
    } else {
  ...
  try {
    ...
    const recommendationsWithReservations = await TableRecommendation.find({
      ...
      .populate(
        ...
        "tableName capacity location ambiance image avgRating"
      )
