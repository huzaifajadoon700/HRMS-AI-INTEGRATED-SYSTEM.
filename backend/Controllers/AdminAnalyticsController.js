// Admin Analytics Controller - Provides comprehensive dashboard analytics and reporting
...
const getDashboardAnalytics = async (req, res) => {
  try {
    ...
    const [
      ...
      Order.find({ createdAt: { $gte: thisMonth } }).select(
        "totalPrice status createdAt items"
      ),
      Order.find({
      ...
      Booking.find().select(
        "totalPrice status createdAt checkInDate checkOutDate roomType roomNumber roomId userId fullName email phone"
      ),
      Booking.find({
        $or: [
      ...
      Reservation.find().select(
        "totalPrice amount price status createdAt reservationDate"
      ),
      Reservation.find({
        $or: [
          { createdAt: { $gte: thisMonth } },
    ...
    const foodRevenue = ordersData.reduce(
      (sum, order) => sum + (order.totalPrice || 0),
      0
    );
    const foodRevenueThisMonth = ordersThisMonth.reduce(
      (sum, order) => sum + (order.totalPrice || 0),
      0
    );
    const foodRevenueLastMonth = ordersLastMonth.reduce(
      (sum, order) => sum + (order.totalPrice || 0),
      0
    );

    console.log("🍽️ Food Revenue:", {
      total: foodRevenue,
      thisMonth: foodRevenueThisMonth,
      lastMonth: foodRevenueLastMonth,
    });
    ...
    const roomRevenue = bookingsData.reduce((sum, booking) => {
      ...
      console.log(
        `🏨 Booking ${booking._id}: Room ${booking.roomNumber} (${booking.roomType}) - Rs.${price} for ${nights} nights`
      );
      return sum + price;
    }, 0);
    ...
    const tableRevenue = reservationsData.reduce((sum, reservation) => {
      return (
        sum +
        (reservation.totalPrice || reservation.amount || reservation.price || 0)
      );
    }, 0);
    const tableRevenueThisMonth = reservationsThisMonth.reduce(
      (sum, reservation) => {
        return (
          sum +
    ...
    const tableRevenueLastMonth = reservationsLastMonth.reduce(
      (sum, reservation) => {
        return (
          sum +
          (reservation.totalPrice ||
    ...
    const totalRevenueThisMonth =
      foodRevenueThisMonth + roomRevenueThisMonth + tableRevenueThisMonth;
    const totalRevenueLastMonth =
      foodRevenueLastMonth + roomRevenueLastMonth + tableRevenueLastMonth;
      ...
      analytics: {
        ...
        revenue: {
          ...
          breakdown: {
            foodPercentage:
              totalRevenue > 0
                ? ((foodRevenue / totalRevenue) * 100).toFixed(1)
                : 0,
            roomsPercentage:
              totalRevenue > 0
            ...
            tablesPercentage:
              totalRevenue > 0
                ? ((tableRevenue / totalRevenue) * 100).toFixed(1)
                : 0,
          },
        },
        rooms: {
        ...
        food: {
          ...
          revenue: foodRevenue,
          avgOrderValue:
            totalOrders > 0 ? (foodRevenue / totalOrders).toFixed(0) : 0,
        },
        tables: {
          total: totalTables,
          ...
          avgReservationValue:
            totalReservations > 0
              ? (tableRevenue / totalReservations).toFixed(0)
              : 0,
        },
        activity: {
          today: {
