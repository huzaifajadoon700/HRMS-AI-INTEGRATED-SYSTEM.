// Recommendation Evaluation Controller - Evaluates ML recommendation accuracy and performance
...
class RecommendationEvaluationController {
  ...
  static async evaluateRecommendationAccuracy(req, res) {
    try {
      ...
      const evaluation =
        await RecommendationEvaluationController.performAccuracyEvaluation(
          userId,
          testPeriodDays
        );
  ...
  static async performAccuracyEvaluation(userId = null, testPeriodDays = 7) {
    try {
      ...
      const startDate = new Date(
        endDate.getTime() - testPeriodDays * 24 * 60 * 60 * 1000
      );
      ...
      metrics.overallAccuracy =
        (metrics.precision + metrics.recall + metrics.ndcg + metrics.hitRate) /
        4;
      ...
      return {
        ...
        dataStats: {
          ...
          uniqueUsers: userId
            ? 1
            : new Set(testInteractions.map((i) => i.userId.toString())).size,
          uniqueItems: new Set(
            testInteractions.map((i) => i.menuItemId._id.toString())
          ).size,
        },
    ...
    try {
      ...
      for (const [userId, userTestInteractions] of userGroups.entries()) {
        ...
        const recommendations =
          await FoodRecommendationController.generateRecommendations(userId, k);
        const recommendedItemIds = new Set(
          recommendations.items.map(
            (item) => item._id?.toString() || item.menuItemId?.toString()
          )
        );
        ...
        const relevantRecommendations = likedItems.filter((itemId) =>
          recommendedItemIds.has(itemId)
        );
        const precision =
          recommendedItemIds.size > 0
            ? relevantRecommendations.length / recommendedItemIds.size
            : 0;
    ...
    try {
      ...
      for (const [userId, userTestInteractions] of userGroups.entries()) {
        ...
        userTestInteractions.forEach((interaction) => {
          relevanceMap.set(
            interaction.menuItemId._id.toString(),
            interaction.rating
          );
        });
        ...
        const sortedRelevances = Array.from(relevanceMap.values()).sort(
          (a, b) => b - a
        );
        let idcg = 0;
  ...
  static async calculateHitRate(
    ...
    try {
      ...
      for (const [userId, userTestInteractions] of userGroups.entries()) {
        ...
        const hasHit = likedItems.some((itemId) =>
          recommendedItemIds.has(itemId)
        );
        if (hasHit) hits++;
        userCount++;
  ...
  static async calculateDiversity(trainingInteractions, testInteractions) {
    try {
      ...
      for (const [userId] of userGroups.entries()) {
        ...
        const diversity =
          (categories.size + cuisines.size) /
          (2 * recommendations.items.length);
